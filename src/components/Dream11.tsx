...
function SubmitView({ selectedPlayers, squad, captain, viceCaptain, constraints, roundId, teamId, onReset }: {
  selectedPlayers: IPLPlayer[];
  squad: string[];
  captain: string | null;
  viceCaptain: string | null;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (token && owner && repo) {
      try {
        ...
        // Fallback: download locally if API fails
      ...
      // No GitHub config — download locally
      const blob = new Blob([csv], { type: 'text/csv' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = filename; a.click();
    ...
    // Lock the squad in localStorage
...
/* ─────────────────── squad builder (3-step) ─────────────────── */

type BuilderStep = 'pick' | 'cvc' | 'submit';

function SquadBuilder({ players, constraints, roundId, teamId }: {
  players: IPLPlayer[];
  constraints: Dream11Constraints;
  roundId: string;
  teamId: string;
    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-2xl font-black" style={{ color: 'var(--text-primary)' }}>Dream11 Fantasy</h1>
        ...
        <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
          Build your fantasy XI · Pick C &amp; VC · Compete across Playoffs → Semis → Finals
        </p>
      </div>
