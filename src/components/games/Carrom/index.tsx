...
export default function Carrom() {
  ...
  const fixtures = useFixtures('data/carrom_fixtures.csv');
  const [tab, setTab] = useState<'fixtures' | 'rules'>('fixtures');

  const game = games?.find(g => g.id === 'carrom');
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black leading-none" style={{ color: 'var(--text-primary)' }}>Carrom</h1>
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Doubles · Round Robin + Knockout</p>
        </div>
      </div>
